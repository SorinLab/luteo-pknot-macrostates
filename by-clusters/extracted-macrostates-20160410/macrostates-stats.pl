...
# ---------------- input & output filenames ------------------------------------
    ...
    #The following is 1 because the values are normalized already.
    $maxS1  = 1;
    $maxS2  = 1;
    $maxL1  = 1;
    $maxL2  = 1;
...
# -------- parsing data --------------------------------------------------------
    ...
    while (my $line = <INPUT>)
    {
        ...
        $statRg->add_data($rg);
        $statS1->add_data($s1/$maxS1);
        $statS2->add_data($s2/$maxS2);
        $statL1->add_data($l1/$maxL1);
        $statL2->add_data($l2/$maxL2);
        $statT->add_data($t/$maxT);
